...
import { api, getApiKey, setApiKey, unwrapJson } from "../../lib/api-client";
...
export function Header() {
  ...
  const capture = useMutation({
    mutationFn: async () => {
      ...
      if (!content) throw new Error("Enter a note.");
      ...
      return unwrapJson<{ note: any; deduped: boolean }>(res);
    },
    onSuccess: (data) => {
  ...
  return (
    <>
      ...
      <ModalShell title="Quick Capture" open={captureOpen} onClose={() => setCaptureOpen(false)}>
        <div className="grid gap-2">
          ...
          <div className="flex items-center justify-end gap-2 pt-2">
            <button
              ...
              className="rounded-[var(--radius-md)] px-3 py-2 text-sm font-semibold text-[var(--text-secondary)] hover:text-[var(--text-primary)]"
            >
              Cancel
            </button>
            <button
              ...
              className="rounded-[var(--radius-md)] bg-[color-mix(in_oklab,var(--accent-task)_22%,transparent)] px-3 py-2 text-sm font-bold text-[var(--text-primary)] hover:bg-[color-mix(in_oklab,var(--accent-task)_28%,transparent)] disabled:opacity-50"
            >
