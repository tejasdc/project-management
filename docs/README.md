...
- [Hosting](clarify-hosting.md): Cloudflare deployment, DNS,
...
Objects and alarms replace paid Postgres/Redis/worker services. Cloudflare is the
only hosting and recovery target. The migration is published:
...
and Workspace database. The browser uses same-origin API calls. There is no Render
fallback or deployment configuration. Final domain acceptance includes
